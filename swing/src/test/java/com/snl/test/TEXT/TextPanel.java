...
public class TextPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        float wrapping = getSize().width - insets.left - insets.right;
        mX = insets.left;
        mY = insets.top;
        lineBreaker = new LineBreakMeasurer(iterator,fr);
        g2.setPaint(Color.MAGENTA);
        while (lineBreaker.getPosition() < iterator.getEndIndex()) {
            int start = lineBreaker.getPosition();
            TextLayout textLayout = lineBreaker.nextLayout(wrapping);
            int end = lineBreaker.getPosition();
            var slice = s.substring(start,end);
            var gv = font.createGlyphVector(fr,slice);
            mY += textLayout.getAscent();
            AffineTransform at = AffineTransform.getTranslateInstance(mX,mY);
            for (int i=0;i<gv.getNumGlyphs();i++) {
                shape = gv.getGlyphOutline(i);
                oldShape = at.createTransformedShape(shape);
                g2.draw(oldShape);
            }
            mY += textLayout.getDescent() + textLayout.getLeading();
        }
        g2.dispose();
    }
    ...
    private void update() {
        ...
        if (now - startTime >= 500L) {
            startTime = now;
            numbers = Math.max(s.length() -1 ,++numbers);
        }
    }

    public static void main(String[] args) {
        SwingUtilities.invokeLater(TextPanel::createUi);
    }
}
